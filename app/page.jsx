        ...
        <div className="flex flex-1 gap-4 overflow-hidden">
          ...
          <aside className="w-64 bg-white shadow-md flex flex-col rounded-2xl overflow-hidden">
            <div className="flex-1 p-4 border-b border-gray-200">
              ...
              <div className="space-y-3">
                <div onClick={() => setVisible((v) => ({ ...v, pie: !v.pie }))}
                ...
                <div onClick={() => setVisible((v) => ({ ...v, bar: !v.bar }))}
                ...
                <div onClick={() => setVisible((v) => ({ ...v, stack: !v.stack }))}
            ...
            <div className="p-4">
              ...
              <ul className="space-y-2">
                {Object.keys(datasets).map((table) => (
                  <li key={table} draggable onDragStart={(e) => handleDragStart(e, table)}
          ...
          <main className="flex-1 overflow-hidden">
            ...
            <div className="relative w-full h-full overflow-auto rounded-2xl overflow-x-hidden overflow-y-hidden">
              {visible.pie && (
                <Rnd bounds="parent" size={pieSize} position={piePos}
                     onDragStop={(_, d) => setPiePos({ x: d.x, y: d.y })}
                     onResizeStop={(_, __, ref, ___, pos) => { setPieSize({ width: ref.offsetWidth, height: ref.offsetHeight }); setPiePos(pos); }}
                  ...
                  {combinedPie.length > 0 ? (
                    <ResponsiveContainer width="100%" height={pieSize.height - 60}>
                      <PieChart>
                        <Pie data={combinedPie} dataKey="xValue" cx="30%" cy="50%" outerRadius={80} fill="#3b82f6" nameKey="name" label>
                          {combinedPie.map((entry, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
                        </Pie>
                        <Pie data={combinedPie} dataKey="yValue" cx="70%" cy="50%" outerRadius={80} fill="#10b981" nameKey="name" label>
                          {combinedPie.map((entry, index) => <Cell key={index} fill={COLORS[(index + 1) % COLORS.length]} />)}
                        </Pie>
                        <Tooltip /><Legend />
                      </PieChart>
                    </ResponsiveContainer>
        ...
        <LineChart data={combinedBarLine}>
          ...
          <Line type="monotone" dataKey="yShipments" stroke="#10b981" strokeWidth={2} />
              ...
              {visible.bar && (
                <Rnd bounds="parent" size={barSize} position={barPos}
                     ...
                     onResizeStop={(_, __, ref, ___, pos) => { setBarSize({ width: ref.offsetWidth, height: ref.offsetHeight }); setBarPos(pos); }}
                     minWidth={300} minHeight={250}
                     className="bg-white rounded-2xl shadow p-4 flex flex-col absolute">
              ...
              {visible.stack && (
                <Rnd bounds="parent" size={stackSize} position={stackPos}
                     onDragStop={(_, d) => setStackPos({ x: d.x, y: d.y })}
                     onResizeStop={(_, __, ref, ___, pos) => { setStackSize({ width: ref.offsetWidth, height: ref.offsetHeight }); setStackPos(pos); }}
                     minWidth={300} minHeight={250}
                     className="bg-white rounded-2xl shadow p-4 flex flex-col absolute">
          ...
          <aside className="w-[380px] bg-white rounded-2xl shadow-md flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {messages.map((m, idx) => (
                <div key={idx}
                     className={`p-2 rounded-lg text-sm ${m.role === 'user' ? 'bg-blue-50 text-gray-800 self-end' : 'bg-gray-100 text-gray-700'}`}>
                  {m.content}
                </div>
              ))}
